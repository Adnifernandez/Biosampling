...
const campaignSchema = z.object({
  ...
  shermanTrapCount: z.coerce.number().int().min(0).optional(),
  cameraTrapCount: z.coerce.number().int().min(0).optional(),
...
export async function createCampaign(projectId: string, formData: FormData) {
  ...
  const parsed = campaignSchema.safeParse({
    ...
    shermanTrapCount: formData.get("shermanTrapCount") ? Number(formData.get("shermanTrapCount")) : undefined,
    cameraTrapCount: formData.get("cameraTrapCount") ? Number(formData.get("cameraTrapCount")) : undefined,
