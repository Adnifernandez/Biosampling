...
import { Search, FolderOpen, MapPin, User2, Pencil, Layers } from "lucide-react";
...
export function ProjectsListClient({ projects, isAdmin }: { projects: Project[]; isAdmin: boolean }) {
  const [query, setQuery] = useState("");

  const q = normalize(query.trim());
  const filtered = q
    ? projects.filter((p) =>
        [p.name, p.region, p.commune, p.createdBy ?? ""].some((field) => normalize(field).includes(q))
      )
    : projects;
          ...
          <CardContent className="py-12 text-center">
            ...
            {projects.length === 0 ? (
              ...
              <>
                ...
                <p className="text-sm text-gray-400 mt-1">Ningún proyecto coincide con &quot;{query}&quot;</p>
              </>
            )}
          </CardContent>
        </Card>
